...
const Components = (() => {
  ...
  function injectHeader() {
    ...
    if (cfg.logo.source === "hosted" && cfg.logo.hostedPath)
      logoHtml = `<img src="${base}${cfg.logo.hostedPath}" alt="${esc(cfg.name)} Logo" onerror="this.outerHTML='<span class=school-logo-text>${cfg.logo.textFallback}</span>'">`;
    else if (cfg.logo.source === "drive" && cfg.logo.driveLink) {
      const m = cfg.logo.driveLink.match(/\/file\/d\/([a-zA-Z0-9_-]+)/);
      if (m) logoHtml = `<img src="https://drive.google.com/uc?export=view&id=${m[1]}" alt="${esc(cfg.name)} Logo" onerror="this.outerHTML='<span class=school-logo-text>${cfg.logo.textFallback}</span>'">`;
    }
      ...
      <div class="container">
        <div class="footer__grid">
          <div class="footer__col footer__brand">
            ...
            <p class="footer__desc">A Government Lower Primary School serving children of Naharkatia since ${cfg.established}, under Samagra Shiksha Abhiyan.</p>
